<?php
if(!class_exists("PayWithMetaMaskForWooCommerceProBase")) {
	class PayWithMetaMaskForWooCommerceProBase {
    	public $key = "F445D7CDC6924574";
    	private $product_id = "17";
    	private $product_base = "pay-with-metamask-for-woocommerce-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_pay-with-metamask-for-woocommerce-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=pay-with-metamask-for-woocommerce-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
		...
		function __plugin_updateInfo(){
            if(function_exists("wp_remote_get")) {
                ...
                if (!is_wp_error($response)) {
                    ...
                    if ( ! $oldFound ) {
                        set_transient( $this->product_base."_up", [ "data" => $this->encrypt( serialize( [ 'body' => $body ] ) ) ], 0 );
                    }
		...
		function PluginUpdate($transient)
		{
			...
			if(!empty($response->plugin)){
                ...
                if (!empty($response) && version_compare(CPMWP_VERSION, $response->new_version, '<')) {
